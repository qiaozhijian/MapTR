        ...
        for record in records:
            ...
            if polygon.is_valid:
                ...
                if not new_polygon.is_empty:
                    ...
                    centerline = list(
                        self.map_api.discretize_lanes([record["token"]], 0.5).values()
                    )[0]
...
def _fill_trainval_infos2(
    ...
    for scene_data in mmcv.track_iter_progress(nusc.scene):
        ...
        for lidar_time in data["LIDAR_TOP"]:
            time_diff = 50000
            frame = {
                "LIDAR_TOP": lidar_time[1],
                "timestamp": lidar_time[0],
    ...
    for sample in mmcv.track_iter_progress(all_frames):
        ...
        map_location = nusc.get("log", nusc.get("scene", scene_token)["log_token"])[
            "location"
        ]
...
class VectorizedLocalMap(object):
    ...
    def get_contour_line(self, patch_box, patch_angle, layer_name):
        ...
        if layer_name == "drivable_area":
            for record in records:
                polygons = [
                    self.map_explorer.map_api.extract_polygon(polygon_token)
    ...
    def centerline_geoms2vec(self, centerline_geoms_list):
        ...
        vectors = self._geom_to_vectors(centerline_geoms_list)
            ...
            if centerline_geom.geom_type == "MultiLineString":
                ...
                for single_geom in centerline_geom.geoms:
                    ...
                    for idx, pt in enumerate(single_geom_pts[:-1]):
                        pts_G.add_edge(
                            tuple(single_geom_pts[idx]), tuple(single_geom_pts[idx + 1])
                        )
...
def create_nuscenes_infos(
    ...
    if test:
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(
            out_path, "{}_map_infos_temporal_train.pkl".format(info_prefix)
        )
        mmcv.dump(data, info_path)
        data["infos"] = val_nusc_infos
        info_val_path = osp.join(
            out_path, "{}_map_infos_temporal_val.pkl".format(info_prefix)
        )
...
def nuscenes_data_prep(
    ...
    create_nuscenes_infos(
        ...
        version=version,
        max_sweeps=max_sweeps,
        use_all_sync_time=use_all_sync_time,
    )
...
parser.add_argument("--use_all_sync_time", default=False, action="store_true")
args = parser.parse_args()

if __name__ == "__main__":

    if args.use_all_sync_time:
        args.out_dir = os.path.join(args.root_path, "custom", "maptrv2_all_sync")
    else:
        args.out_dir = os.path.join(args.root_path, "custom", "maptrv2")
    os.makedirs(args.out_dir, exist_ok=True)

    # Create trainval version
    ...
    nuscenes_data_prep(
        ...
        use_all_sync_time=args.use_all_sync_time,
    )
    ...
    # Merge train, val, test infos
    train_info_file = osp.join(
        args.out_dir, "{}_map_infos_temporal_train.pkl".format(args.extra_tag)
    )
    val_info_file = osp.join(
        args.out_dir, "{}_map_infos_temporal_val.pkl".format(args.extra_tag)
    )
    test_info_file = osp.join(
        args.out_dir, "{}_map_infos_temporal_test.pkl".format(args.extra_tag)
    )
    ...
    info_file = osp.join(
        args.out_dir, "{}_map_infos_temporal_all.pkl".format(args.extra_tag)
    )
    mmcv.dump(all_infos, info_file)
    print(
        "train sample: {}, val sample: {}, test sample: {}, all sample: {}".format(
            len(train_infos), len(val_infos), len(test_infos), len(all_infos["infos"])
        )
    )
